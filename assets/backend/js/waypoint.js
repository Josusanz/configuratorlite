...
(function ($, window) {
    ...
    function stickyWidth() {
        var $imageOptions = $('#wpc-image-options .layers-con');

        if ($imageOptions.length) {
            $imageOptions.width($('#wpc-settings-panel').outerWidth());
        }
    }
    ...
    $(window).load(function () {
        ...
        if ($imageOptions.length) {
            var sticky = new Waypoint.Sticky({
                element: $('#wpc-image-options .layers-con')[0],
                offset: 32,
            });
        }
    });

    $(window).resize(stickyWidth);
})(jQuery, window);
