...
WPC_Utils::get_template(
	...
	array(
		'email_heading' => esc_html__( 'Thank you for your request', 'wp-configurator-pro' ),
...
<p id="wpc-mail-greeting"><?php printf( esc_html__( 'Hi %s,', 'wp-configurator-pro' ), esc_html( $values['name'] ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></p>
<?php /* translators: %s: Order ID */ ?>
<p id="wpc-mail-info"><?php printf( esc_html__( 'Just to let you know — we\'ve received your request [Request #%s], We will contact you shortly.', 'wp-configurator-pro' ), esc_html( $values['request_id'] ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></p>

<div id="wpc-mail-request-details">
<?php /* translators: %s: Request ID, %s: Date */ ?>
<h2><?php printf( esc_html__( '[Request #%1$s] (%2$s)', 'wp-configurator-pro' ), esc_html( $values['request_id'] ), wp_strip_all_tags( current_time( 'F d, Y' ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></h2>
<table width="100%">
	<tbody>
		<tr>
		 ...
		 */
		do_action( 'wpc_customer_request_quote_order_details', $values );
		?>
	</tbody>
</table>
</div>
 ...
 * @param array $values Form details.
 */
do_action( 'wpc_customer_request_quote_after_summary', $values );
...
/**
 * Hook: Before billing details.
 *
 * * @since 2.6
 *
 * @param array $values Form details.
 */
do_action( 'wpc_customer_request_quote_before_billing_details', $values );
?>
</div>
		...
		<tbody>
			<tr>
				<td>
					 ...
					 * @param array $values Form details.
					 */
					do_action( 'wpc_customer_request_quote_billing_details', $values );
					?>
				</td>
			</tr>
		</tbody>
 ...
 * @param array $values Form details.
 */
do_action( 'wpc_customer_request_quote_after_billing_details', $values );
