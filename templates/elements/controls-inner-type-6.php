...
<div class="wpc-control-lists wpc-parent-control wpc-show-control-set" x-bind:class="<?php echo esc_attr( $wpc->store . '.getParentControlWrapperClasses()' ); ?>">
	<?php
	/**
	...
	<ul class="wpc-control-lists-inner" data-controls data-parent-uid="<?php echo esc_attr( $parent_uid ); ?>">
		...
		foreach ( $wpc->components as $top_key => $layer ) {
			...
			if ( isset( $layer['children'] ) ) {
				$sub_controls .= $wpc->get_sub_controls_and_items_separated( $layer['children'], $layer['uid'] );
			}
		}
		?>
	</ul>
...
if ( $sub_controls ) {
	 ...
	 * @param object $wpc WPCSE Class.
	 */
	do_action( 'wpc_before_sub_control_wrapper_html', $wpc );
	?>
	 ...
	 * @param object $wpc WPCSE Class.
	 */
	do_action( 'wpc_after_sub_control_wrapper_html', $wpc );
}
