 ...
 */
class WPC_HtmlPruner extends WPC_AbstractHtmlProcessor
{
    /**
    ...
    const DISPLAY_NONE_MATCHER
        ...
        . ' and not(@class and contains(concat(" ", normalize-space(@class), " "), " -WPC_Emogrifier-keep "))]';
    ...
    {
        $classesToKeepIntersector = new WPC_ArrayIntersector($classesToKeep);
     ...
     *
     * Note that it does not inspect the remaining CSS, but uses information readily available from the `WPC_CssInliner`
     * instance about the CSS rules that could not be inlined.
     *
     * @param WPC_CssInliner $WPC_CssInliner object instance that performed the CSS inlining
     *
     * @return self fluent interface
     *
     * @throws \BadMethodCallException if `inlineCss` has not first been called on `$WPC_CssInliner`
     */
    public function removeRedundantClassesAfterCssInlined(WPC_CssInliner $WPC_CssInliner)
    {
        $classesToKeepAsKeys = [];
        foreach ($WPC_CssInliner->getMatchingUninlinableSelectors() as $selector) {
