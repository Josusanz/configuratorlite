 ...
 *
 * @deprecated Will be removed for version 4.0.0. Please use the WPC_CssInliner class instead.
 *
...
class WPC_Emogrifier
{
    ...
    /**
     * WPC_Emogrifier will throw Exceptions when it encounters an error instead of silently ignoring them.
     *
     * @var bool
     */
    private $debug = false;
    ...
    protected function process()
    {
        ...
        foreach ($cssRules['inlinable'] as $cssRule) {
            ...
            // executions, which can still flood logs/output unnecessarily. Instead, WPC_Emogrifier's error handler should
    ...
    private function copyUninlinableCssToStyleNode(array $cssRules, $cssImportRules)
    {
        ...
        if ($cssRulesRelevantForDocument !== []) {
            ...
            if (!\class_exists(WPC_CssConcatenator::class)) {
                require_once __DIR__ . '/WPC_Emogrifier/Utilities/WPC_CssConcatenator.php';
            }

            $WPC_CssConcatenator = new WPC_CssConcatenator();
            foreach ($cssRulesRelevantForDocument as $cssRule) {
                $WPC_CssConcatenator->append([$cssRule['selector']], $cssRule['declarationsBlock'], $cssRule['media']);
            }

            $css .= $WPC_CssConcatenator->getCss();
        }
     ...
     * just not implemented/recognized yet by WPC_Emogrifier).
     *
     ...
     *
     * @see https://github.com/MyIntervals/WPC_Emogrifier/issues/103
     *
     * @param string $css
     *
     * @return void
